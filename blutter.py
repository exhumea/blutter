...
def find_lib_files(indir: str):
    indir_lib = os.path.join(indir, 'lib')
    if os.path.isdir(indir_lib):
        indir = indir_lib
    indir_arch = os.path.join(indir, SUPPORTED_ARCH)
    if os.path.isdir(indir_arch):
        indir = indir_arch
    app_file = os.path.join(indir, 'libapp.so')
    if not os.path.isfile(app_file):
        #app_file = os.path.join(indir, 'App')
        #if not os.path.isfile(app_file):
        sys.exit(f"Cannot find {app_file}")

    flutter_file = os.path.join(indir, 'libflutter.so')
    if not os.path.isfile(flutter_file):
        #flutter_file = os.path.join(indir, 'Flutter')
        #if not os.path.isfile(flutter_file):
        sys.exit(f"Cannot find {flutter_file}")
...
def main(indir: str, outdir: str | None, rebuild_blutter: bool, create_vs_sln: bool, no_analysis: bool):
    if outdir is None:
        outdir = os.path.join(indir, "blutter_out")
...
if __name__ == "__main__":
    ...
    parser.add_argument('indir', type=str, help='A directory directory that contains both libapp.so and libflutter.so')
    parser.add_argument('outdir', type=str, default=None, nargs='?', help='An output directory (default: $indir/blutter_out/)')
